...
1/ The paper doesn't come with the code so I find it difficult to reproduce the results. I just tried the model in page 14 with 4 covariates only (the number of shots and the number of corners from previous matches) and the outcomes are not really good.

2/ We can use your package to fit and make predictions because it's pretty similar to the number of goals.

3/ Just my very first thoughts on the corner kicks

I used data from the five big leagues (England, Spain, Italy, Germany, France) with 5 latest seasons.

There is kind of a problem with the corner kicks. Since the variance is much greater than the mean, I assume it follows the Negative Binomial distribution. Then I get the parameters, draw samples, make plots and compare to the actual data. As you can see in the images, the NegBin distribution fits the corner kicks of the home team and away team quite well. It's also okay when we calculate the difference of corner kicks. And the issue comes when we sum them up.

You can image I would lose money if I place my bets on corner over/under.

I think it's because these two variables are dependent. The correlation coefficients for the home goals and away goals is -0.094 whereas it's 0.262 for the corner kicks of two teams. 

Do you have any idea to counter this?
